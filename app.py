...
# Handle Vercel / PythonAnywhere database connection fallback
default_db_path = os.path.join(BASE_DIR, 'campus_lf.db')
...
@app.route('/admin/sync_clips', methods=['POST'])
@admin_required
def admin_sync_clips():
    ...
    for filename in os.listdir(clips_dir):
        ...
        # expected format: CAM_01_20260409_215356.mp4
        parts = filename.replace('.mp4', '').replace('_mock', '').split('_')
        if len(parts) >= 4:
            cam_id = parts[0] + "_" + parts[1] # e.g. CAM_01
            date_time_str = parts[2] + "_" + parts[3]
            try:
                ts = datetime.strptime(date_time_str, '%Y%m%d_%H%M%S')
            except ValueError:
                continue # ignore incorrectly formatted files
                
            loc = Location.query.filter_by(camera_id=cam_id).first()
            if not loc:
                continue # Unknown camera
                
            # Check if exists
            exists = CCTVEvent.query.filter_by(clip_path=filepath).first()
            if not exists:
                event = CCTVEvent(
                    camera_id=cam_id,
    ...
    db.session.commit()
    flash(f'Synced clips folder: {added} new clip(s) added.', 'success')
    return redirect(url_for('admin'))
...
def seed_data():
    ...
    if User.query.filter_by(role='admin').count() == 0:
        admin = User(name='Admin', email='admin@campus.com',
